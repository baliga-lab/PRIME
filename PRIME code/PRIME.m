function [RegulatedModel, F_KO, TFs, RefineValue] = PRIME (model, Regulator, Target, Magnitude, GeneExpressionIDs, GeneExpression, overallExpressionid, minOverallExpression, maxOverallExpression)
%%
%PRIME is used to integrate regulatory network onto metabolic networks
%It predicts the phenotype of TF influences
%It also gives the pertubed models as output
%PRIME calculates the magnitude of TF influences on the genes and reactions
%
%USAGE:
%   [RegulatedModel, F_KO, F_OE, TFs] = IDREAM (model, Regulator, Target, Probability, Magnitude, Litevidence)
%
%INPUT:
...
%   GeneExpressionIDs       Gene ID of GE data (present study)
...
for m = 1:length(TFs)
    ...
    RegulatedModel{m,1}.c = inhibit.*min(abs(Refine)); %weight factor
    RegulatedModel{m,1}.ub = maxFlux.*Refine;%max flux perturbation
    RegulatedModel{m,1}.lb = minFlux.*Refine;%min flux perturbation
